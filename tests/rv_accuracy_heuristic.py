...
from main import process_spectrum
...
for i in range(N_TESTS):
    ...
    he = np.random.uniform(low=-5.0, high=2)
...
plt.hist(rv_diffs, bins=50)
plt.tight_layout()
plt.show()

# Define the bins for the histogram
bins = np.linspace(np.min(noise_lvls), np.max(noise_lvls), 10)  # Adjust number of bins as needed

# Initialize arrays to store success counts and total counts for each bin
success_counts = np.zeros(len(bins) - 1)
total_counts = np.zeros(len(bins) - 1)

# Calculate success counts and total counts for each bin
for i in range(len(bins) - 1):
    lower_bound = bins[i]
    upper_bound = bins[i + 1]
    mask = np.logical_and((noise_lvls >= lower_bound),(noise_lvls < upper_bound))
    total_counts[i] = np.sum(mask)
    if total_counts[i] > 0:
        success_counts[i] = np.sum(success[mask]) / total_counts[i] * 100  # Calculate percentage

# Plotting the histogram
plt.figure(figsize=(8, 6))
plt.bar(bins[:-1], success_counts, width=np.diff(bins), align='edge', edgecolor='black')
plt.xlabel('Bins of x')
plt.ylabel('Success Percentage (%)')
plt.title('Success Percentage for Each Bin of x')
plt.grid(True)
plt.show()
