...
from main import pseudo_voigt, slicearr, load_spectrum, v_from_doppler_rel
import pandas as pd
...
for filename in filenames:
    ...
    # plt.savefig("overviewplot.png", dpi=300)
    # plt.show()
    slicedwl, loind, upind = slicearr(wl, LINE_LOC - MARGIN, LINE_LOC + MARGIN)
    flux = flux[loind:upind]
    flux_std = flux_std[loind:upind]
    if CULUMFIT:
    ...
    params = params.loc[params["line_loc"] == LINE_LOC]
    ...
    plt.plot(slicedwl, flux, color=SUBSPECCOLORS[nspec-1])
...
if MANUALLIMITS:
    plt.ylim(YLIM)
    plt.xlim(XLIM)
plt.savefig(f"{SPECFILE_NAME}_shiftzoom_plot.png", dpi=500)
plt.show()
