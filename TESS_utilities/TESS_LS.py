...
def do_tess_stuff(TIC, source_id, folder="lightcurves", plotit=False):
    ...
    #                    "0 = no, 1 = yes: "))
    ### FORCE IT TO SAVE DATA FOR EVERYTHING ####
    flag_lc = 1;
    flag_ls = 1;
    flag_ph = 1;
    flag_p2 = 1
    ...
    if (flag_lc == 1):
        ascii.write([slow_lc.bjd, slow_lc.flux, slow_lc.flux_err],
                    f'{folder}/{TIC}_lc.dat', names=['BJD', 'RelativeFlux', 'Error'],
                    overwrite=True)

    # Calculates the periodogram
    slow_lc.periodogram()
    if (flag_ls == 1):
        ascii.write([1 / slow_lc.freq, slow_lc.power], f'output/{source_id}/ls.dat',
    ...
    if (flag_ph == 1):
        if (flag_p2 == 1):
            ...
            ascii.write([phase, flux_phased, flux_err_phased], f'output/{source_id}/phase.dat',
                        names=['Phase', 'RelativeFlux', 'Error'], overwrite=True)

    if plotit:
        plot = make_plot(slow_lc.freq, np.sqrt(slow_lc.power), slow_lc.fap_001, BJD_or, flux_or,
                         slow_lc.bjd, slow_lc.flux, phase, flux_phased, flux_fit,
                         phase2, flux_phased2, flux_fit2, slow_lc.period, slow_lc.crowdsap,
                         n_slow)
        plot.savefig(f'output/{source_id}/bigplot.png')
    ...
    if fast:
        ...
        if (flag_lc == 1):
            ascii.write([fast_lc.bjd, fast_lc.flux, fast_lc.flux_err],
                        f'{folder}/{TIC}_lc_fast.dat', names=['BJD', 'RelativeFlux', 'Error'],
                        overwrite=True)

        # Calculates the periodogram
        fast_lc.periodogram()
        if (flag_ls == 1):
            ascii.write([1 / fast_lc.freq, fast_lc.power], f'{folder}/ls_fast.dat',
                        names=['Period[h]', 'Power'], overwrite=True)
        ...
        if (flag_ph == 1):
            if (flag_p2 == 1):
                ascii.write([phase2, flux_phased2, flux_err_phased2], f'output/{source_id}/phase_fast.dat',
                            names=['Phase', 'RelativeFlux', 'Error'], overwrite=True)
            else:
                ascii.write([phase, flux_phased, flux_err_phased], f'output/{source_id}/phase_fast.dat',
                            names=['Phase', 'RelativeFlux', 'Error'], overwrite=True)

        plot_fast = make_plot(fast_lc.freq, np.sqrt(fast_lc.power), fast_lc.fap_001, BJD_or, flux_or,
                              fast_lc.bjd, fast_lc.flux, phase, flux_phased, flux_fit,
                              phase2, flux_phased2, flux_fit2, fast_lc.period, fast_lc.crowdsap,
                              n_fast)

        plot_fast.savefig(f'output/{source_id}/bigplot_fast.png')

    ################################

    #########  WRITE LOG  ##########

    log = open(f'{folder}/{TIC}.log', "w")
    log.write(f'{folder}/{TIC}\n\n')
    if warning:
